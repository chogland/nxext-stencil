...
@Component({
  tag: 'rds-date-picker-month',
  styleUrl: 'rds-date-picker-month.scss',
  shadow: true,
})
export class RdsDatePickerMonth {
  //--------------------------------------------------------------------------
  //
  //  Element
  //
  ...
  @Element() el: HTMLRdsDatePickerMonthElement;
   ...
   */
  @Event() rdsDatePickerSelect: EventEmitter;

  /**
   * Event emitted when user hovers the date.
   * @internal
   */
  @Event() rdsDatePickerHover: EventEmitter;

  /**
   * Active date for the user keyboard access.
   */
  @Event() rdsDatePickerActiveDateChange: EventEmitter;

  /**
   * @internal
   */
  @Event() rdsDatePickerMouseOut: EventEmitter;
  ...
  mouseoutHandler(): void {
    this.rdsDatePickerMouseOut.emit();
  }
  ...
  render(): VNode {
    ...
    const weekDays = short || abbreviated || narrow;
  ...
  private addMonths(step: number) {
    ...
    this.rdsDatePickerActiveDateChange.emit(dateFromRange(nextDate, this.min, this.max));
  ...
  private isEndOfRange(date: Date): boolean {
    return (
      ...
      (!this.endDate && this.hoverRange && sameDate(this.startDate, this.hoverRange.end) && sameDate(date, this.hoverRange.end))
    );
  }

  dayHover = (e: CustomEvent): void => {
    const target = e.target as HTMLRdsDatePickerDayElement;
    if (e.detail.disabled) {
      this.rdsDatePickerMouseOut.emit();
    } else {
      this.rdsDatePickerHover.emit(target.value);
    }
  };

  daySelect = (e: CustomEvent): void => {
    const target = e.target as HTMLRdsDatePickerDayElement;
    this.rdsDatePickerSelect.emit(target.value);
  };

  /**
   * Render rds-date-picker-day
   */
  private renderDateDay(active: boolean, day: number, dir: string, date: Date, currentMonth?: boolean, ref?: boolean) {
  ...
  private isRangeHover(date): boolean {
    ...
    const cond1 =
      insideRange && ((!isStart && date > this.startDate && (date < end || sameDate(date, end))) || (isStart && date < this.endDate && (date > start || sameDate(date, start))));
    const cond2 =
      !insideRange &&
      ((!isStart && date >= this.endDate && (date < end || sameDate(date, end))) ||
        (isStart && (date < this.startDate || (this.endDate && sameDate(date, this.startDate))) && (date > start || sameDate(date, start))));
    return cond1 || cond2;
  }
}
