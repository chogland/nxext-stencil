 ...
 */
export function createObserver<T extends ObserverType>(type: T, callback: ObserverCallbackType<T>, options?: ObserverOptions<T>): ObserverInstanceType<T> | undefined {
...
function getObserver<T extends ObserverType>(type: T): ObserverClassType<T> {
  return (type === 'intersection' ? window.IntersectionObserver : type === 'mutation' ? window.MutationObserver : window.ResizeObserver) as any;
}
